...
def write_csv_atomic(path, headers, rows):
    directory = os.path.dirname(path)
    fd, tmp_path = tempfile.mkstemp(prefix=".tmp-", suffix=".csv", dir=directory, text=True)
    try:
...
def color_row(headers, index, values):
    expected = len(headers) - 1
    if len(values) != expected:
        raise ValueError(f"Expected {expected} color columns, got {len(values)}")
    return dict(zip(headers, [str(index)] + values, strict=True))
...
NEW_COLORS = {
    ...
    "Music Creation & Beat Maker": (
        ...
        "#22C55E",
        "#0F172A",
        "Studio purple + waveform green on dark",
    ),
    ...
    "Fasting & Intermittent Timer": (
        ...
        "#059669",
        "#0F172A",
        "Fasting indigo on dark + eating green",
    ),
    # ── F. Social ──
    "Anonymous Community / Confession": (
        ...
        "#0891B2",
        "#0F172A",
        "Protective grey + subtle teal on dark",
    ),
    ...
    "Coding Challenge & Practice": (
        ...
        "#D97706",
        "#0F172A",
        "Code green + difficulty amber on dark",
    ),
    "Kids Learning (ABC & Math)": (
        ...
        "#EC4899",
        "#EFF6FF",
        "Learning blue + play yellow + fun pink",
    ),
    ...
    "Home Decoration & Interior Design": (
        ...
        "#D97706",
        "#FAF5F2",
        "Interior warm grey + gold accent",
    ),
}
...
def rebuild_colors():
    ...
    # Write
    write_csv_atomic(src, headers, final_rows)
