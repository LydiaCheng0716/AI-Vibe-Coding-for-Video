...
def generate_design_system(
    ...
    Args:
        ...
        force: If True, overwrite existing persisted design system files
    ...
    if persist:
        persist_design_system(design_system, page, output_dir, query, force)
...
def _safe_slug(name: str | None, fallback: str = "default") -> str:
    """Convert user-provided names into a safe path segment."""
    slug = re.sub(r"[^a-z0-9_-]+", "-", (name or "").lower().strip()).strip("-")
    return slug or fallback


def _ensure_relative_to_base(path: Path, base: Path) -> Path:
...
def _resolve_output_base(output_dir: str | None = None) -> Path:
    ...
    if not resolved.is_relative_to(cwd):
        raise ValueError(f"Refusing to write outside current working directory: {resolved}")
    return resolved
...
def persist_design_system(
    ...
    """
    base_dir = _resolve_output_base(output_dir)

    # Use project name for project-specific folder
    project_name = design_system.get("project_name", "default")
    project_slug = _safe_slug(project_name)

    design_system_dir = _ensure_relative_to_base(base_dir / "design-system" / project_slug, base_dir)
    pages_dir = _ensure_relative_to_base(design_system_dir / "pages", base_dir)
    ...
    target_files = [master_file]
    page_file = None
    if page:
        page_slug = _safe_slug(page, fallback="page")
        page_file = _ensure_relative_to_base(pages_dir / f"{page_slug}.md", base_dir)
        target_files.append(page_file)

    existing_files = [path for path in target_files if path.exists()]
    if existing_files and not force:
        formatted = ", ".join(str(path) for path in existing_files)
        raise FileExistsError(f"Refusing to overwrite existing design system files without --force: {formatted}")
