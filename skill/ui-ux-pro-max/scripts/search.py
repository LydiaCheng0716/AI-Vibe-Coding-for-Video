...
Domains: style, color, chart, landing, product, ux, typography, google-fonts, web, react, icons
...
from design_system import _resolve_output_base, _safe_slug, generate_design_system


def configure_utf8_stdio():
    """Force UTF-8 for CLI output without mutating streams on import."""
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--persist",
        action="store_true",
        help="Save design system to design-system/<project>/MASTER.md (creates hierarchical structure)",
    )
    parser.add_argument(
        "--page", type=str, default=None, help="Create page-specific override file in design-system/<project>/pages/"
    )
    parser.add_argument(
        "--output-dir",
        "-o",
        type=str,
        default=None,
        help="Output directory for persisted files; must be under current directory (default: current directory)",
    )
    ...
    if args.design_system:
        ...
        if args.persist:
            base_dir = _resolve_output_base(args.output_dir)
            project_slug = _safe_slug(args.project_name or args.query.upper())
            design_system_dir = base_dir / "design-system" / project_slug
            print("\n" + "=" * 60)
            print(f"✅ Design system persisted to {design_system_dir}/")
            print(f"   📄 {design_system_dir / 'MASTER.md'} (Global Source of Truth)")
            if args.page:
                page_filename = _safe_slug(args.page, fallback="page")
