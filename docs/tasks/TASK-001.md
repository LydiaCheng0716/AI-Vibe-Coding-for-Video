...
- [ ] Given 故事文本少于 10 字（按去除首尾空白后的 Unicode 码点数计算），When 用户点击生成，Then 系统应提示故事内容不足以生成分镜。
- [ ] Given 故事文本超过 5000 字（按去除首尾空白后的 Unicode 码点数计算），When 用户点击生成，Then 系统应阻止提交并提示用户缩短故事。
...
- 输入长度硬下限为 10 字、硬上限为 5000 字；按去除首尾空白后的 Unicode 码点数计算，阈值集中配置，避免散落在 UI 逻辑中。
